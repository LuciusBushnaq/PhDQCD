...
void MatrixMatvec(void *x, PRIMME_INT *ldx, void *y, PRIMME_INT *ldy, int *blockSize, primme_params *primme, int *err) {
    ...
    northsouth=malloc(sizeof(complex double)*nxl);
    ...
    for (i=0; i<*blockSize; i++)
    {
        ...
        /* Receives*/
        MPI_Irecv(xvec.buffer[0], nxl, MPI_C_DOUBLE_COMPLEX, mpi_src [0], 101, MPI_COMM_WORLD,
                  &req[0]);
        MPI_Irecv(xvec.buffer[1], nxl, MPI_C_DOUBLE_COMPLEX, mpi_src [1], 102, MPI_COMM_WORLD,
                  &req[1]);
        MPI_Irecv(xvec.buffer[2], nyl, MPI_C_DOUBLE_COMPLEX, mpi_src [2], 103, MPI_COMM_WORLD,
                  &req[2]);
        MPI_Irecv(xvec.buffer[3], nyl, MPI_C_DOUBLE_COMPLEX, mpi_src [3], 104, MPI_COMM_WORLD,
                  &req[3]);
        
        for (j=0;j<nxl;j++) northsouth[j] = xvec.data[j+nxl*(nyl-1)];
        MPI_Send(northsouth, nxl, MPI_C_DOUBLE_COMPLEX, mpi_dest[0], 101, MPI_COMM_WORLD);
        for (j=0;j<nxl;j++) northsouth[j] = xvec.data[j];
        MPI_Send(northsouth, nxl, MPI_C_DOUBLE_COMPLEX, mpi_dest[1], 102, MPI_COMM_WORLD);
        ...
        for (j=0;j<nxl*nyl;j++){
            ...
            yvec[j] = 4.0*xvec.data[j];
            for (i=0;i<4;i++) yvec[j]-=neigh(&xvec,j,i)*1.0;
