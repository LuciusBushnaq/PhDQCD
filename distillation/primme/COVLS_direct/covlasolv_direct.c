...
{
    /* Get (x,y,z)*/
    static const int hop_x3[6] = {0, 0,-1,+1, 0, 0};
    static const int hop_x2[6] = {-1,+1, 0, 0, 0,0};
    static const int hop_x1[6] = {0, 0, 0, 0,-1,+1};
...
complex double config(complex double* U, int site, int hop, int rank, int comp)
{
    ...
    int px1=rank/(np3*np2);
    int px2 = (rank-np3*np2*px1)/np3;
    int px3 = rank % np3;
    ...
    if((x3+x2+x1)%2==0){
        ...
        if(hop==1){
            if(x2<ngx2-1) return conj(U[(x1*ngx2*ngx3/2+(x2+1)*ngx3/2+x3/2)*8*9+5*9+c+3*comp]);
            else return conj(U[(x1*ngx2*ngx3/2+x3/2)*8*9+5*9+c+3*comp]);
        }
        if(hop==2){
        ...
        if(hop==3){
            if(x3<ngx3-1) return conj(U[(x1*ngx2*ngx3/2+x2*ngx3/2+(x3+1)/2)*8*9+7*9+c+3*comp]);
            else return conj(U[(x1*ngx2*ngx3/2+x2*ngx3/2)*8*9+7*9+c+3*comp]);
        ...
        if(hop==4){
            ...
            if(x1<ngx1-1) return conj(U[((x1+1)*ngx2*ngx3/2+x2*ngx3/2+x3/2)*8*9+3*9+c+3*comp]);
            else return conj(U[(x2*ngx3/2+x3/2)*8*9+3*9+c+3*comp]);
        }
    }
    else{
        if(hop==0) return U[(x1*ngx2*ngx3/2+x2*ngx3/2+x3/2)*8*9+5*9+3*c+comp];
        
        if(hop==1) return U[(x1*ngx2*ngx3/2+x2*ngx3/2+x3/2)*8*9+4*9+3*c+comp];
       
        if(hop==2) return U[(x1*ngx2*ngx3/2+x2*ngx3/2+x3/2)*8*9+7*9+3*c+comp];
      
        if(hop==3) return U[(x1*ngx2*ngx3/2+x2*ngx3/2+x3/2)*8*9+6*9+3*c+comp];
            
        
        if(hop==4) return U[(x1*ngx2*ngx3/2+x2*ngx3/2+x3/2)*8*9+3*9+3*c+comp];
        
        else return U[(x1*ngx2*ngx3/2+x2*ngx3/2+x3/2)*8*9+2*9+3*c+comp];
...
{
    int rank,px3,px2,px1;
    MPI_Comm_rank(MPI_COMM_WORLD,&rank);
    
    
    px1=rank/(np3*np2);
    px2 = (rank-np3*np2*px1)/np3;
    px3 = rank % np3;
    
    
    mpi_dest[0]=(px3      )    +np3*((px2+1    )%np2)+np3*np2*px1;
    mpi_src [0]=(px3      )    +np3*((px2-1+np2)%np2)+np3*np2*px1;
    
    mpi_dest[1]=mpi_src[0];
    mpi_src [1]=mpi_dest[0];
    
    mpi_dest[2]=(px3+1    )%np3+np3*((px2      )    )+np3*np2*px1;
    mpi_src [2]=(px3-1+np3)%np3+np3*((px2      )    )+np3*np2*px1;
    
    mpi_dest[3]=mpi_src [2];
    mpi_src [3]=mpi_dest[2];
    
    mpi_dest[4]=(px3      )    +np3*((px2    )    )+np3*np2*((px1+1    )%np1);
    mpi_src [4]=(px3      )    +np3*((px2    )    )+np3*np2*((px1-1+np1)%np1);
...
int main (int argc, char *argv[]) {
    ...
    if (primme.procID == 0) {
        ...
        fprintf(primme.outputFile,"Evecs: %d: %14.6e \n",i,creal(evecs[i*4*8*8*3+((0+1)+(0+2)*4+(0+3)*8*8)*3+1]));
        }
    ...
    if (primme.procID == 1) {
        for (i=0; i < primme.numEvals; i++) {
        fprintf(primme.outputFile,"Evecs from 1: %d: %14.6e \n",i,creal(evecs[i*4*8*8*3+((0+1)+(0+2)*4+(0+3)*8*8)*3+1]));
        }
    }
    primme_free(&primme);
    free(evals);
