...
void MatrixMatvec(void *x, PRIMME_INT *ldx, void *y, PRIMME_INT *ldy, int *blockSize, primme_params *primme, int *err) {
    ...
    eastwest=malloc(sizeof(float)* nyl);
    ...
    {
        xvec.data = (float *)x + *ldx*i;
        yvec      = (float *)y + *ldy*i;
        
        // Receives
        MPI_Irecv(xvec.buffer[1], nxl, MPI_FLOAT, mpi_src [0], 101, MPI_COMM_WORLD,
                  &req[0]);
        MPI_Irecv(xvec.buffer[0], nxl, MPI_FLOAT, mpi_src [1], 102, MPI_COMM_WORLD,
                  &req[1]);
        MPI_Irecv(xvec.buffer[3], nyl, MPI_FLOAT, mpi_src [2], 103, MPI_COMM_WORLD,
                  &req[2]);
        MPI_Irecv(xvec.buffer[2], nyl, MPI_FLOAT, mpi_src [3], 104, MPI_COMM_WORLD,
                  &req[3]);
        
        
        MPI_Send(xvec.data,            nxl,
                 MPI_FLOAT, mpi_dest[0], 101, MPI_COMM_WORLD);
        MPI_Send(xvec.data+nxl*(nyl-1),nxl,
                 MPI_FLOAT, mpi_dest[1], 102, MPI_COMM_WORLD);
        
        for (j=0;j<nyl;j++) eastwest[j] = xvec.data[nxl*j];
        MPI_Send (eastwest,       nyl, MPI_FLOAT, mpi_dest[2], 103, MPI_COMM_WORLD);
        for (j=0;j<nyl;j++) eastwest[j] = xvec.data[nxl-1+nxl*j];
        MPI_Send (eastwest,       nyl, MPI_FLOAT, mpi_dest[3], 104, MPI_COMM_WORLD);
...
int main (int argc, char *argv[]) {
    ...
    nxl = 1000 / npx; nyl = 1000 / npy;
    
    
    primme.n = (nxl*npx)*(nyl*npy); /* set problem dimension */
    // Test here - find all
    primme.numEvals = 20; // primme.n; /* Number of wanted eigenpairs */
    
    primme.eps = .0001;      /* ||r|| <= eps * ||matrix|| */
