...
static void correlators(int nc)
{
    ...
    for (t=0;t<file_head.tmax;t++)
    {
        for(c1=0;c1<dummyv[7];c1++)
        {
            ...
            for(s1=0;s1<4;s1++){
                ...
                data.DV11_dist_re[t]+=corr_re[t]*file_head.nsrc/norm;
                data.DV11_dist_im[t]+=corr_im[t]*file_head.nsrc/norm;
                slices(3,wsd_dist_s[s1],wsd_dist[s1]);
                data.DV22_dist_re[t]+=corr_re[t]*file_head.nsrc/norm;
                data.DV22_dist_im[t]+=corr_im[t]*file_head.nsrc/norm;
                slices(4,wsd_dist_s[s1],wsd_dist[s1]);
                data.DV33_dist_re[t]+=corr_re[t]*file_head.nsrc/norm;
                data.DV33_dist_im[t]+=corr_im[t]*file_head.nsrc/norm;
                slices(5,wsd_dist_s[s1],wsd_dist[s1]);
                data.DVPP_dist_re[t]+=corr_re[t]*file_head.nsrc/norm;
                data.DVPP_dist_im[t]+=corr_im[t]*file_head.nsrc/norm;
            }
        }
    }
    MPI_Barrier(MPI_COMM_WORLD);
    ...
    wtsum+=wt[0];
    if (my_rank==0){
        printf("distilled subspace Dirac solver time %.2e sec\n",wt[0]);
        printf("\n");
        fflush(flog);
    }
    MPI_Barrier(MPI_COMM_WORLD);
    ...
    for(i=0;i<2;++i){
        for (t=0;t<file_head.tmax;t++)
        {
            for (isrc=0;isrc<file_head.nsrc;isrc++)
            {
                ...
                /*Apply distillation operator to stochastic sources, subtract result for rest-space preconditioned sources*/
