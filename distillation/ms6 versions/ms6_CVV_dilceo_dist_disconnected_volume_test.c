...
static void random_source_volume(spinor_dble *eta)
{
    int i,iy,ix;
    double r[24];
    complex_dble *c;
...
static void correlators(int nc)
{
    ...
    for(i=0;i<2;++i){
        for (isrc=0;isrc<file_head.nsrc;isrc++)
        {
            ...
            MPI_Barrier(MPI_COMM_WORLD);
            wt3=MPI_Wtime();
            random_source_volume(wsd_rnd[0]);
            ...
            /*Apply distillation operator to stochastic sources, subtract result for sources projected into distilled space*/
            ...
            for(j=0;j<20;j++){
                for (t=0;t<file_head.tmax;t++)
                {
                    data.alpha_DV11_rest_re[i*20*file_head.tmax+j*file_head.tmax+t]-=(1.0+alpha*j)*corr_re[t]/norm;
                    data.alpha_DV11_rest_im[i*20*file_head.tmax+j*file_head.tmax+t]-=(1.0+alpha*j)*corr_im[t]/norm;
                }
            }
            ...
            for(j=0;j<20;j++){
                for (t=0;t<file_head.tmax;t++)
                {
                    data.alpha_DV22_rest_re[i*20*file_head.tmax+j*file_head.tmax+t]-=(1.0+alpha*j)*corr_re[t]/norm;
                    data.alpha_DV22_rest_im[i*20*file_head.tmax+j*file_head.tmax+t]-=(1.0+alpha*j)*corr_im[t]/norm;
                }
            }
            ...
            for(j=0;j<20;j++){
                for (t=0;t<file_head.tmax;t++)
                {
                    data.alpha_DV33_rest_re[i*20*file_head.tmax+j*file_head.tmax+t]-=(1.0+alpha*j)*corr_re[t]/norm;
                    data.alpha_DV33_rest_im[i*20*file_head.tmax+j*file_head.tmax+t]-=(1.0+alpha*j)*corr_im[t]/norm;
                }
            }
            ...
            for(j=0;j<20;j++){
                for (t=0;t<file_head.tmax;t++)
                {
                    data.alpha_DVPP_rest_re[i*20*file_head.tmax+j*file_head.tmax+t]-=(1.0+alpha*j)*corr_re[t]/norm;
                    data.alpha_DVPP_rest_im[i*20*file_head.tmax+j*file_head.tmax+t]-=(1.0+alpha*j)*corr_im[t]/norm;
                }
            }
