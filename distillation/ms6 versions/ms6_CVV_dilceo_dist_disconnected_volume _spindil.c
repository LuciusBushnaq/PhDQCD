...
static void random_source_volume(spinor_dble *eta, int s)
{
    int i,iy,ix;
    double r[6];
    complex_dble *c;
    ...
    for(i=0;i<2;++i){
        for (isrc=0;isrc<file_head.nsrc;isrc++)
        {
            for (s=0;s<4;s++)
            {
                ...
                MPI_Barrier(MPI_COMM_WORLD);
                wt3=MPI_Wtime();
                random_source_volume(wsd_rnd[0],s);
                ...
                for(c2=0;c2<dummyv[7];c2++){
                    for (t=0;t<file_head.tmax;t++)
                    {
                        ...
                        /*calculate bra-ket product between eigenvector and spin distilled random source*/
                        slices(0,wsd_dist_s[s],wsd_rnd[0]);
                        /*take bra-ket product result times eigenvector, subtract from rest-space preconditioned sources*/
                        convert_dist_source(wsd_rnd[2],wsd_dist_s[s],corr_re[t],corr_im[t],t,1.0,1.0);
                    }
                }
                MPI_Barrier(MPI_COMM_WORLD);
                wt4=MPI_Wtime();
